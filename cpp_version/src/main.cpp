  ...
  //If not using MPI
    ...
    try {
      ...
      * verbose_out << "Starting Ranger." << std::endl;
      ...
      forest -> run(true);
      forest -> writeOutputNewForest(0);

      if (arg_handler.write) {
        forest -> saveToFile(0);
      }
      forest -> writeOutput();
      * verbose_out << "Finished Ranger: i0" << std::endl;

      Data * oldForestData = forest->data;
	...
	forest2->initCppData(arg_handler.depvarname, arg_handler.memmode, arg_handler.file, arg_handler.yfile, arg_handler.mtry, arg_handler.mtryType,
			...
			arg_handler.savemem, arg_handler.splitrule, arg_handler.caseweights, arg_handler.predall, arg_handler.fraction,
			arg_handler.alpha, arg_handler.minprop, arg_handler.holdout, arg_handler.predictiontype,
			arg_handler.randomsplits, arg_handler.useMPI, 0, 0, oldForestData, arg_handler.outputDirectory);
        

        forest2 -> run(true);
        forest2 -> writeOutputNewForest(i);

        if (arg_handler.write) {
           forest2 -> saveToFile(i);
        }
       // forest2 -> writeOutput();
        * verbose_out << "Finished Ranger: i" << i  << std::endl;
